...
pub fn from_fd(config: &Options, window_id: u64, master: OwnedFd, slave: OwnedFd) -> Result<Pty> {
    ...
    #[cfg(any(target_os = "linux", target_os = "macos"))]
    {
        // Convert std OwnedFd to rustix OwnedFd for termios operations
        let rustix_master: RustixOwnedFd = unsafe { <RustixOwnedFd as RustixFromRawFd>::from_raw_fd(master.as_raw_fd()) };
