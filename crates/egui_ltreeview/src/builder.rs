    ...
    fn node_visible_in_clip_rect(&mut self, node: &mut Node<NodeIdType>, outer_rect: Rect) -> f32 {
        ...
        if self.state.is_selected(&node.id) {
            ...
            *rect = Rect::from_min_max(rect.min.min(outer_rect.min), rect.max.max(outer_rect.max));
            ...
            self.ui.painter().set(
                *shape_idx,
                Shape::rect_filled(*rect, self.ui.visuals().widgets.active.rounding, color),
            );
        } else {
        ...
        if self.state.is_dragged(&node.id) {
            self.ui.ctx().layer_painter(self.ui_data.drag_layer).rect_filled(
                outer_rect,
                self.ui.visuals().widgets.active.rounding,
                self.ui.visuals().selection.bg_fill.linear_multiply(0.4),
            );
            // Note: In egui 0.28.1, we can't easily replicate the exact dragging behavior
            // so we'll just draw the selection background on the drag layer
        }
