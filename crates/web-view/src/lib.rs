    ...
    C: AsRef<str>,
{
    ...
    /// Sets the parent window for embedding the WebView.
    ///
    /// When a parent window is provided, the WebView will be embedded as a child
    /// view/window instead of creating a standalone window.
...
// Add thread safety markers for WebView
// Note: This is safe because we ensure single-threaded access in our implementation
...
impl<'a, T> WebView<'a, T> {
    ...
    /// Sets the precise bounds (position and size) of the WebView.
    /// This is useful for positioning embedded WebViews accurately.
    ...
    unsafe fn _safe_drop(&mut self) {
        // Safer drop implementation that avoids potential segfaults
        if let Ok(lock) = self
            .user_data_wrapper()
            .live
            .write()
        {
            ...
            // Skip webview_exit to avoid Objective-C segfaults
            ...
            // Note: We skip webview_exit which contains risky Objective-C calls
            // This may leave some cleanup undone, but prevents segfaults
        } else {
            // If we can't get the lock, just free the webview directly
            webview_free(self.inner.unwrap());
        }
    }
}
...
impl<'a, T> Drop for WebView<'a, T> {
    fn drop(&mut self) {
        if self.inner.is_some() {
            // Use safer drop implementation to avoid segfaults
            unsafe {
                self._safe_drop();
            }
            self.inner = None;
