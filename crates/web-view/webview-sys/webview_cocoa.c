...
WEBVIEW_API webview_t webview_new_embedded(
  const char* title, const char* url,
  int width, int height, int resizable, int debug, int frameless, int visible, int min_width, int min_height, int hide_instead_of_close,
  webview_external_invoke_cb_t external_invoke_cb, void* userdata, void* parent_window) {
	struct cocoa_webview* wv = (struct cocoa_webview*)calloc(1, sizeof(*wv));
	wv->width = width;
	wv->height = height;
	wv->title = title;
...
static void webview_window_will_close(id self, SEL cmd, id notification) {
  ...
  ***/
  id event = ((id(*)(id, SEL, id, id, id, double, id, id, id, id, id))objc_msgSend)((id)objc_getClass("NSEvent"),
                  ...
                  (id)NSApplicationDefinedEvent,
                  (id)objc_getClass("NSZeroPoint"),
                  (id)0, 0.0, (id)0, NULL, (id)0, (id)0, (id)0);
...
static void run_open_panel(id self, SEL cmd, id webView, id parameters,
  ...
  ((id(*)(id, SEL, id))objc_msgSend)(openPanel, sel_registerName("setCanChooseFiles:"), (id)YES);
  ((id(*)(id, SEL, id))objc_msgSend)(
...
static void run_save_panel(id self, SEL cmd, id download, id filename,
  ...
  ((id(*)(id, SEL, id))objc_msgSend)(savePanel, sel_registerName("setCanCreateDirectories:"), (id)YES);
...
static void run_confirmation_panel(id self, SEL cmd, id webView, id message,
  ...
  ((id(*)(id, SEL, id))objc_msgSend)(alert, sel_registerName("setShowsHelp:"), (id)NO);
...
WEBVIEW_API int webview_init(webview_t w) {
  ...
  ((id(*)(id, SEL, id, id))objc_msgSend)(wkPref, sel_registerName("setValue:forKey:"),
               ((id(*)(id, SEL, id))objc_msgSend)((id)objc_getClass("NSNumber"),
                            sel_registerName("numberWithBool:"), (id)(!!wv->debug)),
  ...
  ((id(*)(id, SEL, CGRect, id, id, id))objc_msgSend)(wv->priv.window,
               ...
               r, (id)style, (id)NSBackingStoreBuffered, (id)NO);
  ...
  ((id(*)(id, SEL, id))objc_msgSend)(wv->priv.webview, sel_registerName("setAutoresizesSubviews:"), (id)YES);
  ...
  ((id(*)(id, SEL, id))objc_msgSend)(((id(*)(id, SEL))objc_msgSend)((id)objc_getClass("NSApplication"),
                            sel_registerName("sharedApplication")),
               sel_registerName("activateIgnoringOtherApps:"), (id)YES);
  ...
  ((id(*)(id, SEL, id))objc_msgSend)(item, sel_registerName("setKeyEquivalentModifierMask:"),
               (id)(NSEventModifierFlagOption | NSEventModifierFlagCommand));
  ((id(*)(id, SEL, id))objc_msgSend)(appMenu, sel_registerName("addItem:"), item);
...
WEBVIEW_API void webview_set_color(webview_t w, uint8_t r, uint8_t g,
  ...
  ((id(*)(id, SEL, id))objc_msgSend)(wv->priv.window, sel_registerName("setOpaque:"), (id)NO);
  ((id(*)(id, SEL, id))objc_msgSend)(wv->priv.window,
               sel_registerName("setTitlebarAppearsTransparent:"), (id)YES);
}
...
// Embedded WebView initialization function
WEBVIEW_API int webview_init_embedded(webview_t w, void* parent_window) {
  struct cocoa_webview* wv = (struct cocoa_webview*)w;
  ...
  // Get the parent NSView from the provided window handle
  id parentView = (id)parent_window;
  if (parentView == nil) {
    return -1; // Invalid parent window
  }

  // Validate that parentView is actually an NSView
  Class nsViewClass = objc_getClass("NSView");
  if (!((BOOL(*)(id, SEL, Class))objc_msgSend)(parentView, sel_registerName("isKindOfClass:"), nsViewClass)) {
    return -1; // Not a valid NSView
  }
  ...
  // Create WebView with the specified frame
  CGRect r = CGRectMake(0, 0, wv->width, wv->height);
  wv->priv.webview =
      ((id(*)(id, SEL))objc_msgSend)((id)objc_getClass("WKWebView"), sel_registerName("alloc"));
  ((id(*)(id, SEL, CGRect, id))objc_msgSend)(wv->priv.webview,
  ...
  // Set autoresizing properties
  ((id(*)(id, SEL, id))objc_msgSend)(wv->priv.webview, sel_registerName("setAutoresizesSubviews:"), (id)YES);
  ((id(*)(id, SEL, id))objc_msgSend)(wv->priv.webview, sel_registerName("setAutoresizingMask:"),
               (id)(NSViewWidthSizable | NSViewHeightSizable));

  // Add WebView as subview to the parent view instead of creating a new window
  ((id(*)(id, SEL, id))objc_msgSend)(parentView, sel_registerName("addSubview:"), wv->priv.webview);

  // Store the parent view as our "window" for compatibility
  wv->priv.window = parentView;

  return 0;
}

// Function to set precise WebView position and size
WEBVIEW_API void webview_set_bounds(webview_t w, int x, int y, int width, int height) {
  ...
  // Create new frame with specified bounds
  CGRect newFrame = CGRectMake(x, y, width, height);

  // Simple approach without exception handling to avoid compilation issues
  // Only update window frame if we have a valid window
  if (wv->priv.window != NULL) {
    // Set the frame for the window
    ((id(*)(id, SEL, CGRect, id))objc_msgSend)(wv->priv.window, sel_registerName("setFrame:display:"), newFrame, (id)YES);
  }

  // Only update WebView frame if we have a valid WebView and it's different from window
  if (wv->priv.webview != NULL && wv->priv.webview != wv->priv.window) {
    // Set the frame for the WebView
    ((id(*)(id, SEL, CGRect))objc_msgSend)(wv->priv.webview, sel_registerName("setFrame:"), newFrame);
  }
}
