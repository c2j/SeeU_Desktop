...
impl EmbeddedHelixService {
    ...
    pub async fn new(
        ...
        let client = helix_db::HelixDB::new(Some(config.helix_config.connection_timeout as u16));
...
/// 语义搜索服务实现
impl SemanticSearchService for EmbeddedHelixService {
    /// 索引笔记
    async fn index_note(&self, note: &Note) -> Result<(), SemanticSearchError> {
        // 检查服务状态
        if !self.is_healthy().await {
            return Err(SemanticSearchError::DatabaseError("HelixDB服务不可用".to_string()));
        }
...
impl EmbeddedHelixService {
    /// 解析搜索结果
    fn parse_search_result(&self, result: serde_json::Value, query: &str) -> Option<SemanticSearchResult> {
...
// 重新导出必要的类型
pub use crate::{
    EmbeddingService, SemanticSearchService, SemanticSearchError, HelixDBError,
    Note, SemanticSearchResult, RelatedNote, ConceptExtraction, SearchFilters,
    SemanticSearchConfig, HelixDBManager, HelixDBStatus,
};
