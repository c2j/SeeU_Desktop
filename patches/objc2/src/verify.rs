...
/// Check if two encodings differ only in signedness of an integer type.
///
/// This handles the case where macOS runtime reports 'q' (NSInteger) but
/// the declared type is 'Q' (NSUInteger), or vice versa, which occurs when
/// Swift bridge classes (like ContiguousArray) implement NSFastEnumeration
/// with different type encodings than pure Objective-C classes.
/// See: https://github.com/madsmtm/objc2/issues/566
fn is_sign_mismatch(encoding: &Encoding, expected: &EncodingBox) -> bool {
    matches!(
        (encoding, expected),
        (Encoding::Char, EncodingBox::UChar)
            | (Encoding::UChar, EncodingBox::Char)
            | (Encoding::Short, EncodingBox::UShort)
            | (Encoding::UShort, EncodingBox::Short)
            | (Encoding::Int, EncodingBox::UInt)
            | (Encoding::UInt, EncodingBox::Int)
            | (Encoding::Long, EncodingBox::ULong)
            | (Encoding::ULong, EncodingBox::Long)
            | (Encoding::LongLong, EncodingBox::ULongLong)
            | (Encoding::ULongLong, EncodingBox::LongLong)
    )
}
...
///
/// Also allows signed/unsigned integer type mismatches, which can occur when
/// Swift bridge classes report different type encodings than expected.
fn relaxed_equivalent_to_box(encoding: &Encoding, expected: &EncodingBox) -> bool {
    ...
    {
        true
    } else if is_sign_mismatch(encoding, expected) {
        true
    } else {
