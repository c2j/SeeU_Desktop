...
fn render_notes_settings(ui: &mut egui::Ui, app: &mut SeeUApp) {
    ...
    ui.group(|ui| {
        ui.vertical(|ui| {
            ui.label(egui::RichText::new("语义搜索设置").strong());
            ui.add_space(5.0);

            // 更新语义索引重建状态
            app.inote_state.update_semantic_rebuild_status();

            // 语义搜索状态显示
            ui.horizontal(|ui| {
                ui.label("语义搜索状态:");
                if app.inote_state.semantic_db.is_some() {
                    ui.label(egui::RichText::new("✅ 已启用").color(egui::Color32::from_rgb(0, 150, 0)));
                } else if app.inote_state.semantic_search_enabled {
                    ui.label(egui::RichText::new("🔄 初始化中").color(egui::Color32::from_rgb(255, 165, 0)));
                } else {
                    ui.label(egui::RichText::new("❌ 未启用").color(egui::Color32::from_rgb(200, 0, 0)));
                }
            });
            ...
            ui.horizontal(|ui| {
                ui.label("索引版本:");
                ui.label(egui::RichText::new(&app.inote_state.semantic_index_version).strong());
            });
            ...
            ui.horizontal(|ui| {
                // 重建语义索引按钮
                let rebuild_enabled = !app.inote_state.is_rebuilding_semantic_index;
                let rebuild_button = egui::Button::new("🔄 重建语义索引")
                    .fill(if rebuild_enabled {
                        ui.style().visuals.widgets.active.bg_fill
                    } else {
                        ui.style().visuals.widgets.inactive.bg_fill
                    });

                let rebuild_response = ui.add_enabled(rebuild_enabled, rebuild_button)
                    .on_hover_text("重新构建所有笔记的语义向量索引，用于升级语义模型后增强搜索匹配能力");

                if rebuild_response.clicked() {
                    app.inote_state.start_semantic_index_rebuild();
                }

                // 清除语义索引按钮
                let clear_enabled = !app.inote_state.is_rebuilding_semantic_index;
                let clear_response = ui.add_enabled(clear_enabled, egui::Button::new("🗑️ 清除索引"))
                    .on_hover_text("清除所有语义索引数据，释放存储空间");

                if clear_response.clicked() {
                    app.inote_state.clear_semantic_index();
                }
            });
            ...
            // 显示重建进度
            if app.inote_state.is_rebuilding_semantic_index {
                ui.horizontal(|ui| {
                    ui.spinner();
                    ui.label("正在重建语义索引...");
                });
                ...
                // 显示进度条（如果有进度信息）
                if let Some(progress) = app.inote_state.semantic_rebuild_progress {
                    ui.add(egui::ProgressBar::new(progress).text(format!("{:.1}%", progress * 100.0)));
                }
            }

            // 显示重建结果
            if let Some(result) = &app.inote_state.semantic_rebuild_result {
                match result {
                    Ok(stats) => {
                        ui.label(egui::RichText::new(format!(
                            "✅ 重建完成: {} 个笔记已重新索引",
            ...
            // 语义搜索设置选项
            let mut semantic_settings_changed = false;

            if ui.checkbox(&mut app.inote_state.use_semantic_search_by_default, "默认使用语义搜索")
                .on_hover_text("启用后，搜索时将优先使用语义搜索而非传统关键词搜索")
                .changed() {
                semantic_settings_changed = true;
            }

            if semantic_settings_changed {
                // 保存语义搜索设置
                if let Err(err) = inote::save_settings(&app.inote_state) {
                    log::error!("Failed to save semantic search settings: {}", err);
                } else {
                    log::info!("语义搜索设置已更新并保存");
                }
            }
        });
    });
